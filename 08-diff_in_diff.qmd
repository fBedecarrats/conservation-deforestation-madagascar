...
On lance l'analyse pour les données par mailles. 
```{r}
library(sf)
library(wdpar)
...
# Reprendre les dates de création

infos_vahatra <- AP_Vahatra %>%
  st_drop_geometry() %>%
  select(nom_wdpa, cat_iucn_vahatra = cat__iucn, date_creation) %>%
  mutate(an_creation_vahatra = year(date_creation)) %>%
  select(-date_creation)

date_statut <- WDPA_Mada %>%
  st_drop_geometry() %>%
  select(id_wdpa= WDPAID, nom_wdpa = NAME, cat_iucn_wdpa = IUCN_CAT, 
         an_creation_wdpa = STATUS_YR) %>%
  left_join(infos_vahatra, by = "nom_wdpa") %>% # Garde catégorie selon Vahatra
  mutate(cat_iucn = ifelse(!is.na(cat_iucn_vahatra),  cat_iucn_vahatra, 
                           cat_iucn_wdpa),
         an_creation = ifelse(!is.na(an_creation_vahatra), an_creation_vahatra, 
                              an_creation_wdpa)) %>%
  select(id_wdpa, nom_wdpa, cat_iucn, an_creation)
...
grille_mada_summary <- grille_mada %>%
  ...
  grille_mada_summary2 <- grille_mada_summary %>%
