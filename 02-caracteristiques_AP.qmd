...
```{r}
if (file.exists("data_s3/Vahatra_poly.rds")) {
  load("data_s3/Vahatra_poly.rds")
} else {
  ...
  save(Vahatra_poly, file = "data_s3/Vahatra_poly.rds")
}

Vahatra2 <- Vahatra_poly %>%
  unnest(cols = c(tri, elevation, traveltime)) %>%
  unnest(treecover_area) %>%
  pivot_wider(names_from = "years", names_prefix = "treecover_area_",
              values_from = "treecover") 
